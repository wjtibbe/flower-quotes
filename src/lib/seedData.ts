/**
 * Seed data for local development, demos and first-time production setup.
 *
 ...
 *
 * This is called both by `prisma/seed.ts` (local dev: `npm run db:seed`)
 * and by the one-time `/api/admin/seed` route (production: no terminal
 * needed, see README).
 */
import {
  PrismaClient,
...
export async function seedDatabase(prisma: PrismaClient): Promise<string> {
  ...
  if (existingUsers > 0) {
    return "Database bevat al gebruikers - seed is overgeslagen om duplicaten te voorkomen.";
  }
  ...
  const willemJan = await prisma.user.create({
    data: {
      name: "Willem-Jan",
  ...
  const rose = await prisma.product.create({
    data: {
      ...
      variants: {
        create: [
          { variety: "Be Sweet", stemLength: "70" },
          { variety: "Explorer", stemLength: "70" },
          { variety: "Freedom" },
          { variety: "Vendela", stemLength: "60" },
          { variety: "Mondial" },
  ...
  return `Seed voltooid. Gebruikers: ${mike.email}, ${willemJan.email} (wachtwoord: Welkom2026! - wijzig dit na eerste login).`;
}
